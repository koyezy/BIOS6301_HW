...
*Due Thursday, 09 September, 1:00 PM*

50 points total.

Add your name as `author` to the file's metadata section.

Submit a single knitr file (named `homework1.rmd`) by email to michael.l.williams@vanderbilt.edu.
...
exercise <- factor(c('moderate','frequent','some','some','moderate',
...
```
gender seems to have the highest effect on 'los' considering its largest absolute value. 
```
...
```
The standard errors are 1.4741 and 1.0987 for gender and intercept respectively.
```
...
```{r}
pt(mod.c[,3], df = 7, lower.tail = FALSE) * 2 
```
...
```
```
Yes, the p-value is 0.0224 which matches the p value for gender from summary(mod)
```
